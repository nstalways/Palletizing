...
from palletizing_utilities import calCenter, makePulse, Py2Ard
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(description='Palletizing Robot Evaluation') # parser 객체 생성
    parser.add_argument('--trained_model',
                        default='weights/ssd300_mAP_77.43_v2.pth', type=str,
                        help='Trained state_dict file path to open. If "interrupt", this will open the interrupt file.') # parser 객체가 입력받을 인수 추가
    ...
    parser.add_argument('--score_threshold', default=0, type=float,
                        help='Detections with a score under this threshold will not be considered. This currently only works in display mode.') # IoU threshold?
    ...
    global args
    args = parser.parse_args(argv) # 입력받은 인수들을 분석해서 args에 저장
...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    ...
    with timer.env('Postprocess'):
        ...
        t = postprocess(dets_out, w, h, visualize_lincomb = args.display_lincomb,
                                        ...
                                        score_threshold   = args.score_threshold) # t: [classes, scores, boxes, masks]
        cfg.rescore_bbox = save

    with timer.env('Copy'):
        idx = t[1].argsort(0, descending=True)[:args.top_k] # 계산한 score를 args.top_k개 만큼 내림차순으로(Max ~ Min) 정렬 후 index를 계산
        
        if cfg.eval_mask_branch:
            # Masks are drawn on the GPU, so don't copy
            masks = t[3][idx] # 계산한 index를 바탕으로 masks를 가져옴. idx가 여러 개라면, 모두 가져옴.
            # all_objects_mask = t[3][:args.top_k] # https://stackoverflow.com/questions/64440857/get-the-polygon-coordinates-of-predicted-output-mask-in-yolact-yolact 추가한 부분
        classes, scores, boxes = [x[idx].cpu().numpy() for x in t[:3]]

    print(classes.shape)
    num_dets_to_consider = min(args.top_k, classes.shape[0]) # 현재 classes는 cubic 한 개임.
    # confidence score를 기반으로 고려할 detection 개수 정하는 부분
    ...
    if args.display_masks and cfg.eval_mask_branch and num_dets_to_consider > 0:
        # After this, mask is of size [num_dets, h, w, 1] -> 조건을 만족해서 살아남은 mask들
        ...
        for i in range(num_dets_to_consider):
            ...
            if cnts == []:
                ...
                # contourArea가 제일 넓은 contour를 선택하여, 중심점을 계산한다.
                global M
                M = cv2.moments(cnt)
                out = np.zeros(img_numpy.shape, np.uint8)
            ...
            # image_id = self.bbox_data["image_id"]
                ...
                cv2.drawContours(out, [approx], -1, 255, cv2.FILLED)
                out = cv2.bitwise_and(img_numpy, out)
            #return (out)
...
def evalimage(net:Yolact, img, save_path:str=None):
    ...
    preds = net(batch) # type(preds) == list
...
def evaluate(net:Yolact, dataset, img, train_mode=False): # dataset은 None으로 들어감. 
    ...
    try:
        ...
        for it, image_idx in enumerate(dataset_indices):
            ...
            with timer.env('Network Extra'):
                preds = net(batch)
                '''print(preds.shape) # check shape of segmentation results'''
...
if __name__ == '__main__':
    ...
    while True:
        ...
        while True:
            ...
            # yolact는 정해진 규격의 이미지만 받기 때문에, frame의 size를 늘려주어야 함.
            frame = imutils.resize(frame, width = 960)

            if not ret:
                break
            
            # 정해진 위치 내에만 물체를 두도록 frame에 boundary를 표시
            cv2.imshow('Camera', cv2.rectangle(frame, (203, 83), (756, 636), (0, 0, 255), thickness = 2))

            # 메인 알고리즘
            if cv2.waitKey(1) == ord('c'): # 사용자가 10ms 이내에 c버튼을 누르면
                state = True
                global object_img
                object_img = frame[85:635,205:755] # object가 찍혀있는 이미지
                print('이미지가 캡처되었습니다.')
                cv2.destroyAllWindows()
        ...
        if state is False:
            ...
            # object_img를 학습된 yolact에 넘겨주어야 함.
            print('인공지능을 이용하여 물체를 감지합니다...')
            ...
            with torch.no_grad():
                ...
                print('감지 결과입니다.')
                evaluate(net, dataset, object_img) # 현재 evaluate 결과가 아무것도 return하지 않음.
                

            # 예측한 Mask
            print(f'Mask shape: {out.shape}')
            plt.imshow(out) # shape: (550, 550), type: np.array
            plt.title('Mask')
            plt.show(block=False)
            plt.pause(3)
            plt.close()
