...
// 동작 원리
// 1. while (Serial.available()) 구문을 통해 입력받은 시리얼 데이터가 있다면 while문으로 들어간다.
// 2. 입력받은 시리얼 데이터를 기준에 맞게 split하고, 변수에 저장한다.
// 3. 저장한 변수를 바탕으로 모터를 제어한다.
// 3-1. 먼저 wrist를 제어한다(로봇의 손목 부분을 구동).
// 3-2. 이후 z축 모터를 제어한다(펄스 개수만큼 z축으로 이동).
// 3-3. 마지막으로 그리퍼를 제어한다(펄스 값에 따라 그리퍼가 움직임).
